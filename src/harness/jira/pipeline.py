...
from harness.jira import (
    find_parent_projects,
    findings_hierarchy_plan,
    findings_to_jira,
    subp_vuln_rollup,
)
...
class PipelineLock:
    ...
    def __enter__(self) -> PipelineLock:
        ...
        if self.path.exists():
            ...
            if age_seconds <= self.stale_seconds:
                ...
                raise PipelineFailure(
                    "Another Jira pipeline run appears active. "
                    f"Lock details: {json.dumps(details, sort_keys=True)}",
                    EXIT_LOCKED,
                )
        ...
        try:
            ...
            raise PipelineFailure(
                f"Pipeline lock was acquired concurrently: {self.path}",
                EXIT_LOCKED,
            ) from error
...
def invoke_module_main(
    ...
    raise RuntimeError(
        f"{module.__name__}.main() returned "
        f"unsupported value: {result!r}"
    )
...
def run_stage(
    ...
    try:
        ...
        raise PipelineFailure(
            f"Stage {name!r} raised an exception: {error}",
            EXIT_PARTIAL,
        ) from error
    ...
    if exit_code != 0:
        stage_result["status"] = "failed"
        raise PipelineFailure(
            f"Stage {name!r} exited with code {exit_code}",
            exit_code,
        )
    ...
    if missing_outputs:
        ...
        raise PipelineFailure(
            f"Stage {name!r} did not create required output(s): "
            + ", ".join(missing_outputs),
            EXIT_REQUIRED_OUTPUT_MISSING,
        )
    ...
    print(
        ...
        f"in {stage_result['elapsed_seconds']}s"
    )
...
def ensure_empty_rollup_failure_report(
        path: Path,
) -> None:
    if path.exists():
        return
...
def load_config(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise PipelineFailure(
            f"Jira configuration does not exist: {path}",
            EXIT_ARGUMENT_ERROR,
        )
    ...
    try:
        ...
        raise PipelineFailure(
            f"Jira configuration is invalid: {error}",
            EXIT_ARGUMENT_ERROR,
        ) from error

    if not isinstance(payload, dict):
        raise PipelineFailure(
            "Jira configuration must contain a JSON object",
            EXIT_ARGUMENT_ERROR,
        )
...
def validate_environment(
    ...
    try:
        ...
        raise PipelineFailure(
            f"Output directory is not writable: {data_root}: {error}",
            EXIT_ARGUMENT_ERROR,
        ) from error
    finally:
    ...
    if missing_blackduck:
        raise PipelineFailure(
            "Missing required Black Duck environment variable(s): "
            + ", ".join(missing_blackduck),
            EXIT_ARGUMENT_ERROR,
        )
    ...
    if not isinstance(jira_config, dict):
        raise PipelineFailure(
            "jira configuration must be an object",
            EXIT_ARGUMENT_ERROR,
        )
    ...
    if not project_key:
        raise PipelineFailure(
            "jira.project_key must be configured",
            EXIT_ARGUMENT_ERROR,
        )
    ...
    if args.ca_bundle:
        ...
        if not ca_path.is_file():
            raise PipelineFailure(
                f"Configured CA bundle does not exist: {ca_path}",
                EXIT_ARGUMENT_ERROR,
            )
    ...
    if args.apply:
        jira_url = str(
            os.getenv("JIRA_URL")
            or jira_config.get("url")
            or ""
        ).strip()

        if not jira_url:
            raise PipelineFailure(
                "JIRA_URL or jira.url is required for apply mode",
                EXIT_ARGUMENT_ERROR,
            )

        auth_mode = str(
            jira_config.get("auth_mode") or "basic"
        ).strip().lower()

        if auth_mode == "bearer":
            if not os.getenv("JIRA_PAT"):
                raise PipelineFailure(
                    "JIRA_PAT is required for Jira bearer auth",
                    EXIT_ARGUMENT_ERROR,
                )
        elif not (
            os.getenv("JIRA_USER")
            ...
            raise PipelineFailure(
                "JIRA_USER and JIRA_API_TOKEN are required "
                "for Jira basic auth",
                EXIT_ARGUMENT_ERROR,
            )
    ...
    return {
        ...
        "tls_mode": (
            ...
            if args.insecure
                ...
                f"custom-ca:{args.ca_bundle}"
                if args.ca_bundle
                else "verified-system-ca"
            )
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the complete Black Duck to Jira hierarchy "
            "pipeline once and exit."
        )
    )
    ...
    mode.add_argument(
        "--dry-run",
        action="store_true",
        help="Build the Jira plan without applying Jira changes.",
    )
    mode.add_argument(
        "--apply",
        action="store_true",
        help="Apply the generated hierarchy to Jira.",
    )
    ...
    failure_mode.add_argument(
        ...
        dest="strict",
        action="store_true",
        help="Reject Jira publishing when Black Duck failures exist.",
    )
    failure_mode.add_argument(
        "--allow-partial",
        dest="strict",
        action="store_false",
        help=(
            "Publish successful findings despite Black Duck "
            "failures and return a partial exit status."
        ),
    )
    ...
    tls_mode.add_argument(
        "--ca-bundle",
        help="Customer root/intermediate CA bundle path.",
    )
    tls_mode.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for non-production testing.",
    )
    ...
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="Validate configuration, storage, and secrets only.",
    )
    parser.add_argument(
        "--refresh-parents",
        action="store_true",
        help="Force a full parent relationship rescan.",
    )
    parser.add_argument(
        "--refresh-blackduck-cache",
        action="store_true",
        help="Refresh the vulnerability API cache.",
    )
    parser.add_argument(
        "--refresh-existing-jira",
        action="store_true",
        help="Reconcile local Jira state against Jira.",
    )
    parser.add_argument(
        "--sync-existing-fields",
        action="store_true",
        help="Update configured managed fields on existing issues.",
    )
    parser.add_argument(
        "--max-create",
        type=int,
        help="Maximum Jira issues to create during apply.",
    )
    parser.add_argument(
        "--only-parent-project",
    )
    ...
    parser.add_argument(
        ...
        help="Optional parent discovery project-name filter.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    try:
        with PipelineLock(
            ...
            summary["preflight"] = validate_environment(
                args,
                data_root,
            )
            ...
            parent_cache = (
                cache_dir / "parent_projects_cache.json"
            )
            rollup_cache = (
                cache_dir / "subp_vuln_rollup_cache.json"
            )
            jira_state = (
                state_dir / "jira-rollup-state.json"
            )

            parent_output = run_dir / "parent_projects.csv"
            parent_changes = (
                run_dir / "parent_project_changes.csv"
            )
            ...
            parent_failure_count = count_parent_cache_failures(
                parent_cache
            )
            summary["failure_counts"][
                "parent_scan_failures"
            ] = parent_failure_count

            findings_output = run_dir / "findings.csv"
            rollup_failures = (
                run_dir / "subp_vuln_rollup_failures.csv"
            )
            ...
            ensure_empty_rollup_failure_report(
                rollup_failures
            )
            rollup_failure_count = count_csv_rows(
                rollup_failures
            )
            summary["failure_counts"][
                "rollup_failures"
            ] = rollup_failure_count
            save_summary()

            partial_detected = bool(
                parent_failure_count
                or rollup_failure_count
            )

            if partial_detected and args.strict:
                raise PipelineFailure(
                    "Strict mode rejected Jira publishing because "
                    f"parent failures={parent_failure_count}, "
                    f"rollup failures={rollup_failure_count}",
                    EXIT_STRICT_REJECTION,
                )

            hierarchy_plan = (
                run_dir / "jira-hierarchy-plan.json"
            )
            hierarchy_summary = (
                run_dir / "jira-hierarchy-summary.csv"
            )
            hierarchy_nodes = (
                run_dir / "jira-hierarchy-nodes.csv"
            )
            ...
            plan_payload = json.loads(
                hierarchy_plan.read_text(encoding="utf-8")
            )
            plan_nodes = plan_payload.get("nodes")

            if not isinstance(plan_nodes, list):
                raise PipelineFailure(
                    "Generated hierarchy plan has no nodes array",
                    EXIT_REQUIRED_OUTPUT_MISSING,
                )

            summary["hierarchy_counts"] = (
                plan_payload.get("node_counts", {})
            )

            jira_results = (
                run_dir / "jira-rollup-results.csv"
            )
            jira_publish_plan = (
                run_dir / "jira-rollup-plan.json"
            )
            ...
            summary["promoted_outputs"] = promote_outputs(
                run_dir,
                active_dir,
            )
        ...
        summary["status"] = (
            "skipped_publish"
            if error.exit_code == EXIT_STRICT_REJECTION
            else "failed"
        )
        ...
        save_summary()
        print(
            f"ERROR: unexpected pipeline failure: {error}",
            file=sys.stderr,
        )
        return EXIT_PARTIAL
